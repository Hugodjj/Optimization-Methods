...
def update_velocities_positions_deepso_pb(positions, velocities, global_best_position, wi, wp, wc, lower_limit, upper_limit, communication_rate, mutation_rate, best_individuals, function, step_size):
    for i in range(len(positions)):
        dimension = len(positions[i])

        Xr1_index = randint(0, len(best_individuals) - 1)
        Xr = best_individuals[Xr1_index]
...
def deepso_pb(function, dimension, num_particles, lower_limit, upper_limit, wi, wp, wc, max_iter,communication_rate, mutation_rate, step_size):
    ...
    for _ in range(max_iter):
        ...
        positions, velocities = update_velocities_positions_deepso_pb(positions, velocities, global_best_position, wi, wp, wc, lower_limit, upper_limit, communication_rate, mutation_rate, best_individuals, function, step_size)
